...
int pointsN;	//Общее количество узлов
...
void genSet()
{
	...
	for (i = 0; i < pointsN; i++)		//создаём массив с координатами узлов
...
{
	int i;
	cout << "Количество точек:" << pointsN << endl;
	for (i = 0; i < pointsN; i++)
...
//Вроде работает, но возможно немного неправильно
...
void reg_usel(int i)
{
    ...
    //вычислили h(i) и h(i-1) по Х
    hx = points[i+1][0] - points[i][0];	//h(i)
    hx1 = points[i][0] - points[i-1][0];	//h(i-1)

    A[i][3] = -1.0*(2.0/(hx*hx1) + gamma(points[i][0],points[i][1]));//вычисляем U(i,j)
    A[i][2] = 2.0/(hx1*(hx+hx1));//вычисляем U(i-1,j)
    A[i][4] = 2.0/(hx*(hx+hx1));//вычисляем U(i+1,j)

    //вычислили h(i) и h(i-1) по Y
	t = (area.nx2+1)*(area.ny1+1);
		//если узел находится в нижнем прямоугольнике
		if(i<(t-area.nx2-1)){
				hy = points[i + (area.nx2+1)][1] - points[i][1];
                hy1 = points[i][1] - points[i - (area.nx2+1)][1];
                A[i][1] = 2.0/(hy1*(hy+hy1));//вычисляем Ui,j-1
				A[i][5] = 2.0/(hy*(hy+hy1));}//вычисляем Ui,j+1
		//если узел на стыке двух прямоугольников
		if(i<(t+area.nx1) && i>=(t-area.nx2-1)){
            ...
            A[i][1] = 2.0/(hy1*(hy+hy1));//вычисляем Ui,j-1
            A[i][6] = 2.0/(hy*(hy+hy1));//вычисляем Ui,j+1
        }
		//верхний прямоугольник
		if(i>t){
			hy = points[i+area.nx1+area.nx2+area.nx3+1][1] - points[i][1];
			hy1 = points[i][1] - points[i-(area.nx1+area.nx2+area.nx3+1)][1];
            A[i][0] = 2.0/(hy1*(hy+hy1));//вычисляем Ui,j-1
            A[i][6] = 2.0/(hy*(hy+hy1));//вычисляем Ui,j+1
            } 

    A[i][3] += -2.0/(hy*hy1);//вычисляем Ui,j y-я добавка
...
void create_SLAU()
{
	...
	{ 
        obnulenie(i); 
        F[i] = 0.;
        if((p=kr_poisk(i))==-1) //если i-я вершина не на границе
            reg_usel(i);      //то обрабатываем ее как регулярную
        else
...
int main()
{
	...
	output_grid();
	create_SLAU();
	//надо убрать эти выводу в файл. они меня бесят))
	output_matrix();
	New_input_matrix_1();
	Jakoby(1);
	WriteRes();
